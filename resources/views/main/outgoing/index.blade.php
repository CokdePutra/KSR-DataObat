            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <table class="table table-hover table-striped" id="tableData">
                        ...
                        <tbody>
                            @foreach ($outgoingMedicines as $outgoingMedicines)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $outgoingMedicines->outgoing_date }}</td>
                                    <td>{{ $outgoingMedicines->description }}</td>
                                    {{-- <td>
                                        <span
                                            class="badge {{ $outgoingMedicines->is_active == true ? 'badge-primary' : 'badge-danger' }}">{{ $outgoingMedicines->is_active == true ? 'Active' : 'Non-Active' }}</span>
                                    </td> --}}
                                    {{-- @can('petugas') --}}
                                        <td>
                                            <a href="{{route('outgoing.edit', $outgoingMedicines->id)}}">
